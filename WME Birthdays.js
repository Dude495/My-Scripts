...
// ==/UserScript==
// Original code to pull editor names by technical_01 (Lines 20-21)
...
(function() {
    ...
    const BlackList = /(argus-cronos)/img //Users who wish to opt-out
    ...
    function custMessage() {
        if (DBG == false) {
            ...
            console.info('Birthdays for PM: \n\n'+copyText.replace(BlackList,''));
            let Slice1 = arrBirthdayList.slice(0, 20).join('%0A%0D')
            let PMList1 = CUSTPMLink + Slice1.replace(BlackList,'');
            window.open(PMList1);
            if(arrBirthdayList.length > 20){
                let Slice2 = arrBirthdayList.slice(0, 20).join('%0A%0D')
                let PMList2 = CUSTPMLink + Slice2.replace(BlackList,'');
                window.open(PMList2);
                if (arrBirthdayList.length > 40){
                    let Slice3 = arrBirthdayList.slice(0, 20).join('%0A%0D')
                    let PMList3 = CUSTPMLink + Slice3.replace(BlackList,'');
                    window.open(PMList3);
                }
            }
        }
    ...
    function createMessage() {
        if (DBG == false) {
            ...
            console.info('Birthdays for PM: \n\n'+copyText.replace(BlackList,''));
            let Slice1 = arrBirthdayList.slice(0, 20).join('%0A%0D')
            let PMList1 = PMLink + Slice1.replace(BlackList, '');
            window.open(PMList1);
            if(arrBirthdayList.length > 20){
                let Slice2 = arrBirthdayList.slice(20, 40).join('%0A%0D')
                let PMList2 = PMLink + Slice2.replace(BlackList, '');
                window.open(PMList2);
                if (arrBirthdayList.length > 40){
                    let Slice3 = arrBirthdayList.slice(40, 60).join('%0A%0D')
                    let PMList3 = PMLink + Slice3.replace(BlackList, '');
                    window.open(PMList3);
                }
            }
        }
    ...
    function createBMessage() {
        if (DBG == false) {
            ...
            console.info('Birthdays for PM: \n\n'+copyText.replace(BlackList,''));
            let Slice1 = arrBirthdayList.slice(0, 20).join('%0A%0D')
            let PMList1 = BPMBlink + Slice1.replace(BlackList, '');
            window.open(PMList1);
            console.log('PMList1 Triggered (BPM)');
            if(arrBirthdayList.length > 20){
                let Slice2 = arrBirthdayList.slice(20, 40).join('%0A%0D')
                let PMList2 = BPMBlink + Slice2.replace(BlackList, '');
                window.open(PMList2);
                console.log('PMList2 Triggered (BPM)');
                if (arrBirthdayList.length > 40){
                    let Slice3 = arrBirthdayList.slice(40, 60).join('%0A%0D')
                    let PMList3 = BPMBlink + Slice3.replace(BlackList, '');
                    window.open(PMList3);
